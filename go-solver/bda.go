...
func generateBDA(preset *Config, buildID string) []Item {
	if it := profileOverride(buildID); it != nil {
		return it
	}
	identity := NewSession()
	...
	var enhanced []Item
	if arkFirefox() {
		enhanced = buildEnhancedFPFirefox(preset, identity, device, buildID)
	} else {
	...
	var fHash string
	if arkFirefox() {
		feArr, fHash = buildFEFirefox(preset, device, identity)
	} else {
...
func buildEnhancedFP(preset *Config, identity *DeviceIdentity, device DeviceProfile, buildID string) []Item {
	...
	return append(append([]Item{}, webglFields...), []Item{
		...
		{Key: "screen_pixel_depth", Value: device.ColorDepth}, // was *3 (72); real=colorDepth (24)
		{Key: "navigator_device_memory", Value: clampMem(device.DeviceMemory)}, // Chrome caps at 8
		...
		{Key: "6a62b2a558", Value: firstNonEmpty(buildID, preset.EnforcementHash, defaultEnforcementHash)}, // real enforcement build hash
