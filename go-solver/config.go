...
func (c *Config) applyDefaults() {
	...
	if c.StyleTheme == "" {
		c.StyleTheme = envOrDefault("STYLETHEME", "default")
	}
	if c.Language == "" {
		c.Language = "en-US"
	}
	...
	if c.WindowAncestorOrigins == nil {
		c.WindowAncestorOrigins = []string{"https://www.roblox.com"}
	}
	if c.WindowTreeIndex == nil {
		c.WindowTreeIndex = []int{0}
	}
	if c.WindowTreeStructure == "" {
		c.WindowTreeStructure = "[[],[],[],[[]],[]]"
	}
	if c.DocumentReferrer == "" {
		c.DocumentReferrer = "https://www.roblox.com/"
	}
...
// envOrDefault lets the caller match style_theme/capi_mode to the SOURCE of a
// replayed BDA ciphertext. Real Roblox sends capi_mode=lightbox style_theme=modal;
// our Camoufox harness sends inline/default. They must agree with the ciphertext.
